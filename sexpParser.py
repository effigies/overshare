...
from pyparsing import ParseResults, ParseFatalException, \
    Forward, OneOrMore, Optional, ZeroOrMore, Group, Suppress, \
    Regex, Word, \
    removeQuotes, \
    alphanums, dblQuotedString, hexnums, printables
    ...
    if t.len is not None:
        ...
        if t1len != t.len:
            raise ParseFatalException(
                s, l, "invalid data of length {:d}, expected {}".format(t1len,
                                                                        t.len))
    return t[1]
...
hexadecimal = ("#" + OneOrMore(Word(hexnums)) + "#").setParseAction(
